...
sys.path.insert(0, "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/mmpose-main")
...
sys.path.insert(0, "/home/ubuntu/MyFiles/haoran/code/Music-Source-Separation-Training")
# sys.path.append("/home/ubuntu/MyFiles/haoran/code/music_seperatio/clearvoice")
...
from yolov91.yolov9.utils1.general import (LOGGER, Profile, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
...
GROUNDING_DINO_CONFIG = "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/Grounded-SAM-2-main/grounding_dino/groundingdino/config/GroundingDINO_SwinT_OGC.py"
GROUNDING_DINO_CHECKPOINT = "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/Grounded-SAM-2-main/gdino_checkpoints/groundingdino_swint_ogc.pth"
...
def load_model_pose(face_only: bool = True):
    ...
    if face_only:
        def pose_face(img):
            ...
            scores = scores_out[0] if isinstance(scores_out, (list, tuple)) else scores_out
...
def run_one(path, model, opt, config, device, verbose=False):
    ...
    for mix in track_proc_list:
        waveforms = demix(config, model, mix, device, pbar=not opt.disable_detailed_pbar, model_type="mel_band_roformer")
...
def process_item_person_2(model_yolo, vid_path, vr, saved_vid_root, grounding_model, image_predictor, video_predictor, tag, BOX_THRESHOLD_1):
    # 初始化视频预测状态
...
def visualize(vid_path,video_segments,fps,out_width,out_height,save_vis_file_path,vr,OBJECTS):

    writer_vis = (ffmpeg
            .input('pipe:', format='rawvideo', pix_fmt='rgb24',
                    s='{}x{}'.format(out_width, out_height), r=fps)
            .output(save_vis_file_path, vcodec='libx264', pix_fmt='yuv444p',
                    video_bitrate="10M", r=fps)
            .overwrite_output()
    ...
    for frame_idx, segments in video_segments.items():
        ...
        annotated_frame = box_annotator.annotate(scene=vr[frame_idx].numpy().copy(), detections=detections)
...
import subprocess
import shlex
def visualize_tracks(video_reader,
                     person_bbox_dict,
                     all_speaker_data,
                     output_path,
                     fps,
...
def calculate_mouth_movement(keypoints, prev_mouth_position=None):
    ...
    if prev_mouth_position is not None:
        mouth_movement = np.linalg.norm(mouth_keypoints.mean(axis=0) - prev_mouth_position)

    return mouth_opening, mouth_movement, mouth_keypoints.mean(axis=0)


def track_for_landmarks(tracks,vr,):
    pass
...
class Worker:
    def __init__(self):
        ...
        self.model_yolo = YOLO11_ONNX("/home/ubuntu/MyFiles/haoran/code/Data_process_talking/ultralytics/yolo11n.onnx")
    ...
    def save_and_enhance_results(self, data, output_dir):
        ...
        for instr in data['instruments']:
            ...
            if instr == 'vocals':
                ...
                # 调用 ClearVoice 进行增强
                output_wav = self.myClearVoice(input_path=output_file, online_write=False)
    ...
    def detect_mouth_keypoint(self, vidtracks,ii):
        ...
        pose_config = "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/mmpose-main/configs/face_2d_keypoint/rtmpose/face6/rtmpose-m_8xb256-120e_face6-256x256.py"
        pose_checkpoint = "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/mmpose-main/configs/face_2d_keypoint/rtmpose-m_simcc-face6_pt-in1k_120e-256x256-72a37400_20230529.pth"
        ...
        for frame_idx, frame in enumerate(vidtracks):
            ...
            frame_vis = draw_keypoints(frame.copy(), keypoints, pred_instances["keypoint_scores"], kp_thr=0.5)
            mmcv.imwrite(frame_vis, f'/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/img/111_frame_{frame_idx}_{ii}_valid.png')

            mouth_opening, mouth_movement, current_mouth_position = calculate_mouth_movement(keypoints, prev_mouth_position)
            ...
            print(f"Frame {frame_idx}: Mouth Opening: {mouth_opening}, Mouth Movement: {mouth_movement}")

            if valid_frames >= movement_time_window and total_mouth_movement >= movement_threshold:
                frame_vis = draw_keypoints(frame.copy(), keypoints, pred_instances["keypoint_scores"], kp_thr=0.5)
                mmcv.imwrite(frame_vis, f'/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/img/111_frame_{frame_idx}_{ii}_valid.png')
                print("嘴巴有足够的运动痕迹")
    ...
    '''
    def second_step(self, video_path, audio_path, file_name, opt, grounding_model, image_predictor, video_predictor, model_yolo, pipeline,pose_model):
        ...
        person_bbox_dict, bbox_mask,video_segments,OBJECTS = process_item_person_2(model_yolo, video_path, vr, opt.saved_vid_root+"/person", grounding_model, image_predictor, video_predictor,"face",BOX_THRESHOLD)
        ...
        for start_time, end_time, speaker_id in time_speaker:
            ...
            for pid, track in alltracks:
                ...
                for fidx, bbox in track:
                    if fidx < start_frame or fidx > end_frame or fidx < 0 or fidx >= len(vr):
                        continue
                ...
                frame_idx = [fidx for fidx, _ in track if start_frame <= fidx <= end_frame]
                ...
                # 判断syncnet结果
                if opt.min_offset <= offset <= opt.max_offset and conf >= opt.min_confidence:
                    if conf > best_conf:
                        best_conf = conf
        ...
        if valid:
            ...
            visualize_tracks(video_reader=vr,person_bbox_dict=person_bbox_dict,all_speaker_data=all_speaker_data,output_path=vis_path,fps=fps)
            ...
            with open(opt.output_csv_path, 'a', newline='') as f:
                writer = csv.writer(f)
                if f.tell() == 0:
                    writer.writerow(['audio_path','total_frames','mask_jsonl','pose_jsonl','video_path',"visualize_tracks"])
                writer.writerow(
            ...
            failed_dir = os.path.join(opt.saved_vid_root, "failed_videos_tvshow_batch-1")
    ...
    def __call__(self, item):
        ...
        try:
            ...
            self.second_step(vid_path, output_wav, file_name, opt, self.grounding_model, self.image_predictor, self.video_predictor, self.model_yolo,self.pipeline,self.pose_model)
            item['status'] = [1]
        except Exception as e:
...
parser.add_argument('--input_csv_path', default='/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/2-input_3.5~5.5.csv')
parser.add_argument('--output_csv_path', default='/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/out_4_22_final_2.csv')
parser.add_argument('--initial_syncnet', type=str, default='/home/ubuntu/MyFiles/ruidi/ClipValidation/data/syncnet_v2.model', help='Path to the SyncNet model.')
parser.add_argument('--face_record',  type=str, default='/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/out_4_22_final_2/face_record',  help='path of face_detection jsonl file saved')
parser.add_argument('--store_dir', default="/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/out_4_22_final_2/saved_vid_root/store_dir")
parser.add_argument('--saved_vid_root', default='/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/out_4_22_final_2/saved_vid_root/')
parser.add_argument('--log_path', default='/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_tvshow/data/batch-1/out_4_22_final_2/log/test.log', help="日志地址")
parser.add_argument("--ray_log_dir", type=str, default="/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_virtual/log/ray_log/batch-1/log/ray_log_nice/")

parser.add_argument("--det-config", 
                    default="/home/ubuntu/MyFiles/liujie/sapiens/pose/demo/mmdetection_cfg/rtmdet_m_640-8xb32_coco-person_no_nms.py", 
                    help="Config file for detection")
parser.add_argument("--det-checkpoint", 
                    default="/home/ubuntu/MyFiles/liujie/sapiens/ckpts/sapiens/sapiens_lite_host/torchscript/detector/checkpoints/rtmpose/rtmdet_m_8xb32-100e_coco-obj365-person-235e8209.pth", 
...
parser.add_argument('--min_confidence',  type=int, default=1.0,  help='Minimum confidence value')
...
#     else:
#         csv_path = "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_teethandmouth/data/batch-1/test.csv"
#         ds = ray.data.read_csv(csv_path)
#         # 第一阶段：读取视频数据，分配 CPU 资源
#         ds = ds.map_batches(stage1_read, num_cpus=1,concurrency=1)
#         # # 第二阶段：算法处理阶段
#         ds = ds.map_batches(stage2_algorithm, num_gpus=1, batch_size=1,concurrency=1)
#         # # 第三阶段：输出结果阶段
#         ds = ds.map_batches(stage3_output, batch_format="pyarrow", num_cpus=1)
#         ds.write_json("/home/ubuntu/MyFiles/haoran/code/Data_process_talking/process_for_teethandmouth/log/log1.jsonl")
            ...
            runtime_env={
                "env_vars": {
                    "PYTHONPATH": ":".join([
                        ...
                        "/home/ubuntu/MyFiles/haoran/code/Data_process_talking/mmpose-main",
                        "/home/ubuntu/MyFiles/haoran/code/Music-Source-Separation-Training",
                        "/home/ubuntu/MyFiles/haoran/code/syncnet_python-master",
                        "/home/ubuntu/MyFiles/ruidi/syncnet_python",
                    ]),
